import sharp from "sharp";
import { mkdir, writeFile } from "node:fs/promises";
import { dirname } from "node:path";

const OUT = "public";

const SUN_PATHS = [
  "M12 2.25C12.4142 2.25 12.75 2.58579 12.75 3V5.25C12.75 5.66421 12.4142 6 12 6C11.5858 6 11.25 5.66421 11.25 5.25V3C11.25 2.58579 11.5858 2.25 12 2.25Z",
  "M7.5 12C7.5 9.51472 9.51472 7.5 12 7.5C14.4853 7.5 16.5 9.51472 16.5 12C16.5 14.4853 14.4853 16.5 12 16.5C9.51472 16.5 7.5 14.4853 7.5 12Z",
  "M18.8943 6.16634C19.1872 5.87344 19.1872 5.39857 18.8943 5.10568C18.6014 4.81278 18.1266 4.81278 17.8337 5.10568L16.2427 6.69667C15.9498 6.98956 15.9498 7.46444 16.2427 7.75733C16.5356 8.05022 17.0105 8.05022 17.3034 7.75733L18.8943 6.16634Z",
  "M21.75 12C21.75 12.4142 21.4142 12.75 21 12.75H18.75C18.3358 12.75 18 12.4142 18 12C18 11.5858 18.3358 11.25 18.75 11.25H21C21.4142 11.25 21.75 11.5858 21.75 12Z",
  "M17.8336 18.8943C18.1265 19.1871 18.6013 19.1871 18.8942 18.8943C19.1871 18.6014 19.1871 18.1265 18.8942 17.8336L17.3032 16.2426C17.0103 15.9497 16.5355 15.9497 16.2426 16.2426C15.9497 16.5355 15.9497 17.0104 16.2426 17.3033L17.8336 18.8943Z",
  "M12 18C12.4142 18 12.75 18.3358 12.75 18.75V21C12.75 21.4142 12.4142 21.75 12 21.75C11.5858 21.75 11.25 21.4142 11.25 21V18.75C11.25 18.3358 11.5858 18 12 18Z",
  "M7.7575 17.3033C8.0504 17.0104 8.0504 16.5355 7.7575 16.2426C7.46461 15.9497 6.98974 15.9497 6.69684 16.2426L5.10585 17.8336C4.81296 18.1265 4.81296 18.6014 5.10585 18.8943C5.39875 19.1872 5.87362 19.1872 6.16651 18.8943L7.7575 17.3033Z",
  "M6 12C6 12.4142 5.66421 12.75 5.25 12.75H3C2.58579 12.75 2.25 12.4142 2.25 12C2.25 11.5858 2.58579 11.25 3 11.25H5.25C5.66421 11.25 6 11.5858 6 12Z",
  "M6.69673 7.75732C6.98962 8.05021 7.4645 8.05021 7.75739 7.75732C8.05028 7.46443 8.05028 6.98955 7.75739 6.69666L6.1664 5.10567C5.87351 4.81278 5.39863 4.81278 5.10574 5.10567C4.81285 5.39856 4.81285 5.87344 5.10574 6.16633L6.69673 7.75732Z",
];

const CLOUD_PATH =
  "M4.5 9.75C4.5 6.43629 7.18629 3.75 10.5 3.75C13.028 3.75 15.1893 5.31282 16.0733 7.52408C16.2135 7.50816 16.3559 7.5 16.5 7.5C18.5711 7.5 20.25 9.17893 20.25 11.25C20.25 11.4459 20.2349 11.6386 20.2058 11.827C21.5744 12.5981 22.5 14.0653 22.5 15.75C22.5 18.2353 20.4853 20.25 18 20.25H6.75C3.85051 20.25 1.5 17.8995 1.5 15C1.5 12.8968 2.73627 11.084 4.52024 10.2459C4.50683 10.0822 4.5 9.91686 4.5 9.75Z";

function glyph(scale) {
  const paths = SUN_PATHS.map((d) => `<path d="${d}"/>`).join("");
  return `<g transform="translate(256 256) scale(${scale}) translate(-256 -256)">
    <g transform="translate(60 34) scale(9.6)" fill="#ffd24c">${paths}</g>
    <g transform="translate(150 205) scale(13)" fill="#eef1f5"><path d="${CLOUD_PATH}"/></g>
  </g>`;
}

function svg({ rounded, scale }) {
  const background = rounded
    ? '<rect width="512" height="512" rx="112" fill="url(#bg)"/>'
    : '<rect width="512" height="512" fill="url(#bg)"/>';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512">
  <defs>
    <linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0" stop-color="#4c8df6"/>
      <stop offset="1" stop-color="#0b57d0"/>
    </linearGradient>
  </defs>
  ${background}
  ${glyph(scale)}
</svg>`;
}

const targets = [
  { file: `${OUT}/icons/icon-192.png`, size: 192, svg: svg({ rounded: true, scale: 1 }) },
  { file: `${OUT}/icons/icon-512.png`, size: 512, svg: svg({ rounded: true, scale: 1 }) },
  { file: `${OUT}/icons/maskable-512.png`, size: 512, svg: svg({ rounded: false, scale: 0.72 }) },
  { file: `${OUT}/apple-touch-icon.png`, size: 180, svg: svg({ rounded: false, scale: 0.86 }) },
];

await mkdir(`${OUT}/icons`, { recursive: true });
for (const target of targets) {
  await mkdir(dirname(target.file), { recursive: true });
  await sharp(Buffer.from(target.svg))
    .resize(target.size, target.size)
    .png()
    .toFile(target.file);
}

await writeFile(`${OUT}/favicon.svg`, svg({ rounded: true, scale: 1 }));

console.log("iconos generados con heroicons");
